...
def get_connection():
    """
    Conexão PostgreSQL central do SGFE WEB.
    Cada request recebe uma conexão nova e começa com search_path=public.
    """
    ...
    try:
        ...
        cur.close()
        print("[SGFE-DB] PostgreSQL conectado | schema=public", flush=True)
        return conn
    except Exception:
        try:
            conn.rollback()
...
@app.route("/api/saude-db")
def api_saude_db():
    """Diagnóstico da comunicação SGFE WEB -> PostgreSQL."""
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor()
        ...
        """)
        tabelas = int((cur.fetchone() or [0])[0] or 0)
        ...
        for tabela in (
            ...
            try:
                ...
                dados[tabela] = int((cur.fetchone() or [0])[0] or 0)
            except Exception as exc:
                db_rollback(conn)
                dados[tabela] = f"ERRO: {exc}"

        return jsonify({
            "ok": True,
            "database": db,
            "schema": schema,
            "tabelas_public": tabelas,
            "dados": dados
        })
    except Exception as exc:
        db_rollback(conn)
...
def api_evento_geral():
    ...
    Os indicadores são calculados diretamente pelas tabelas operacionais,
    sem depender de JOINs com tabelas de status. Isso evita que uma
    diferença de estrutura da migração faça todos os indicadores virarem 0.
    """
    conn = None
    resumo = {
        "agendamentos": 0,
    ...
    try:
        ...
        print("[SGFE-API] EVENTO-GERAL V4 - INICIO", flush=True)
        ...
        # O HTML envia ?evento=ID.
        evento_raw = request.args.get("evento", "").strip()
        evento_id = (
            access_int(evento_raw)
            if evento_raw
            else (eventos[0]["id"] if eventos else 0)
        )

        print(
            f"[SGFE-API] EVENTO-GERAL V4 - evento recebido={evento_raw!r} "
            f"evento usado={evento_id}",
            flush=True
        )
            ...
            if r:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    print(
                        f"[SGFE-API] V4 agendamentos ERRO: {exc}",
                        flush=True
                    )
                ...
                try:
                    cur.execute("""
                        SELECT COUNT(*), COALESCE(SUM(ValorFinal), 0)
                        FROM tblVendaPacotes
                        WHERE IDEvento=?
                    """, [evento_id])

                    r = cur.fetchone() or [0, 0]
                    resumo["vendas"] = int(r[0] or 0)
                    resumo["total_vendido"] = float(r[1] or 0)

                    print(
                        f"[SGFE-API] V4 vendas={resumo['vendas']} "
                        f"total_vendido={resumo['total_vendido']}",
                        flush=True
                    )
                except Exception as exc:
                    try:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    print(
                        f"[SGFE-API] V4 entregas ERRO: {exc}",
                        flush=True
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    print(
                        f"[SGFE-API] V4 recebido ERRO: {exc}",
                        flush=True
                    )

                # =================================================
                # 7. PAGAMENTOS PENDENTES
                # Uma OS do evento sem pagamento registrado.
                # =================================================
                try:
                    cur.execute("""
                        SELECT COUNT(*)
                        FROM tblVendaPacotes AS V
                        WHERE V.IDEvento=?
                          AND (V.StatusPagamento IS NULL
                               OR LOWER(V.StatusPagamento)='aberto')
                          AND NOT EXISTS (
                              SELECT 1
                              FROM tblPagamento AS P
                    ...
                    resumo["pagamentos_pendentes"] = int(
                        (cur.fetchone() or [0])[0] or 0
                    )
                except Exception as exc:
                    try:
                        ...
                        pass
                    print(
                        f"[SGFE-API] V4 pagamentos ERRO: {exc}",
                        flush=True
                    )
                ...
                try:
                    cur.execute("""
                        SELECT COALESCE(SUM(ValorDespesa), 0)
                        FROM tblDespesasEvento
                        WHERE IDEvento=?
                    """, [evento_id])

                    resumo["despesas"] = float(
                        (cur.fetchone() or [0])[0] or 0
                    )
                except Exception as exc:
                    try:
                        ...
                        pass
                    print(
                        f"[SGFE-API] V4 despesas ERRO: {exc}",
                        flush=True
                    )
        ...
        print(
            "[SGFE-API] EVENTO-GERAL V4 OK | "
            f"evento={evento_id} | "
            f"agendamentos={resumo['agendamentos']} | "
            f"vendas={resumo['vendas']} | "
            f"entregas={resumo['entregas_pendentes']} | "
            f"pagamentos={resumo['pagamentos_pendentes']} | "
            f"vendido={resumo['total_vendido']:.2f} | "
            f"recebido={resumo['total_recebido']:.2f} | "
            f"despesas={resumo['despesas']:.2f} | "
            f"lucro={resumo['lucro']:.2f}",
            flush=True
        )
        ...
        print(
            f"[SGFE-API] EVENTO-GERAL V4 ERRO GERAL: {e}",
            flush=True
        )
