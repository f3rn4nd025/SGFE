...
(function(){
    ...
    function ocultarColunaFinalizado(){
        /* Mantem FINALIZADO no codigo/dados para preservar a logica.
           Apenas oculta visualmente a coluna na tabela de Vendas/Pacotes. */
        document.querySelectorAll('table').forEach(function(table){
            const ths=Array.from(table.querySelectorAll('thead th'));
            let indice=-1;
            ths.forEach(function(th,i){
                if(texto(th).toUpperCase()==='FINALIZADO') indice=i;
            });
            if(indice<0) return;
            ...
            table.querySelectorAll('tbody tr').forEach(function(tr){
                ...
                if(celulas[indice]) celulas[indice].style.display='none';
            });
        });
    }
