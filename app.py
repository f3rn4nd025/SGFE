...
(function(){
    ...
    function organizarTabela(){
        // Obsoleto: a tabela de Vendas/Pacotes agora já vem pronta do
        // servidor (só as 8 colunas visíveis, sem DATA/EVENTO/FINALIZADO/
        // DATA FINALIZAÇÃO/AÇÃO antiga), com largura controlada 100% por
        // CSS (classes vendas-wrap/vendas-table). Não há mais nada pra
        // esconder ou reconstruir aqui via JavaScript.
        return;
    }

    function organizarBotoes(){
        // Deixa a área de botões de ação do topo (ex.: + NOVA VENDA)
        // quebrar linha de forma organizada em telas estreitas.
        document.querySelectorAll('.actions').forEach(function(el){
            el.classList.add('sgfe-vendas-top-actions');
        });
    }
