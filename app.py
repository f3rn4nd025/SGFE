...
def api_evento_geral():
    ...
    CORREÇÃO EXCLUSIVA DO PAINEL:
    no PostgreSQL migrado do Access, alguns IDs foram importados como
    VARCHAR. Por isso, as comparações de IDs desta rota são feitas como
    texto somente quando estamos no PostgreSQL. Nenhuma outra rota é alterada.
    """
    conn = None
    try:
        conn = get_connection()
        ...
        # O banco PostgreSQL recebido da migração possui alguns campos de ID
        # como VARCHAR. A versão antiga comparava VARCHAR com INTEGER e o
        # PostgreSQL interrompia a transação antes de devolver os eventos.
        pg = _is_postgres()
        id_eq = (lambda campo: f"CAST({campo} AS TEXT)=CAST(? AS TEXT)") if pg else (lambda campo: f"{campo}=?")
        id_join = (lambda a, b: f"CAST({a} AS TEXT)=CAST({b} AS TEXT)") if pg else (lambda a, b: f"{a}={b}")
        ...
        if evento_id:
            ...
            if r:
                ...
                resumo["agendamentos"] = int(scalar(
                    cur,
                    f"SELECT Count(*) FROM tblAgendamentos WHERE {id_eq('IDEvento')}",
                    0,
                    [evento_id],
                ) or 0)
                ...
                resumo["entregas_pendentes"] = sum(
                    1 for r in vendas_evento if r[2] is None or not bool(r[2])
                )
                ...
                cur.execute(f"""
                    ...
                    INNER JOIN tblVendaPacotes AS V
                        ON {id_join('P.IDVenda', 'V.IDVenda')}
                    WHERE {id_eq('V.IDEvento')}
                    GROUP BY P.IDVenda
                """, [evento_id])
                for r in cur.fetchall():
                ...
                try:
                    resumo["despesas"] = money(scalar(
                        cur,
                        f"SELECT Sum(ValorDespesa) FROM tblDespesasEvento WHERE {id_eq('IDEvento')}",
                        0,
                        [evento_id],
                    ))
                except Exception:
        ...
        print(f"[SGFE-PAINEL-ERRO] rota=/api/evento-geral erro={e}", flush=True)
        return jsonify({"ok": False, "erro": str(e)}), 500
    finally:
        if conn:
            conn.close()
