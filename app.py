...
def web_vendas():
    ...
    try:
        ...
        # DIAGNÓSTICO: não altera a consulta nem os dados.
        # Mostra exatamente como IDEvento está chegando e quantas vendas
        # existem antes/depois do filtro selecionado.
        try:
            cur_diag = conn.cursor()
            cur_diag.execute("SELECT IDVenda, IDEvento, pg_typeof(IDEvento)::text FROM tblVendaPacotes ORDER BY IDVenda DESC LIMIT 20")
            print("[SGFE-VENDAS-REAL] evento_selecionado=", evento_selecionado)
            print("[SGFE-VENDAS-REAL] IDEvento_reais=", cur_diag.fetchall())
            cur_diag.close()
        except Exception as e:
            print("[SGFE-VENDAS-REAL] diagnostico_erro=", repr(e))
