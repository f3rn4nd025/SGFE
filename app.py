...
def api_evento_geral():
    ...
    A lista de eventos e os indicadores usam as mesmas relações
    operacionais já utilizadas pelas telas de Entregas/Pagamentos.
    """
    conn = None
    resumo = {
        "agendamentos": 0,
    ...
    try:
        ...
        print("[SGFE-API] EVENTO-GERAL V3 - LISTANDO EVENTOS", flush=True)
        ...
        if evento_id:
            ...
            if r:
                ...
                # -------------------------------------------------
                # 3. AGENDAMENTOS
                # Fonte: tblAgendamentos, diretamente pelo evento.
                # -------------------------------------------------
                try:
                    cur.execute("""
                        SELECT COUNT(*)
                    ...
                    """, [evento_id])
                    r_ag = cur.fetchone()
                    resumo["agendamentos"] = int(r_ag[0] or 0) if r_ag else 0
                except Exception as exc:
                    try:
                        conn.rollback()
                    except Exception:
                        pass
                    print(f"[SGFE-API] agendamentos: {exc}", flush=True)

                # -------------------------------------------------
                # 4. VENDAS / OS
                # Usa a mesma relação da tela ENTREGAS.
                # -------------------------------------------------
                vendas_evento = []
                try:
                    cur.execute("""
                        ...
                        FROM (((tblVendaPacotes AS V
                            ...
                            LEFT JOIN tblStatusVenda AS S
                                ON V.IDStatusVenda=S.IDStatusVenda)
                        WHERE V.IDEvento=?
                          AND (
                              S.StatusVenda Is Null
                              OR UCASE(S.StatusVenda) NOT LIKE '%CANCEL%'
                          )
                    ...
                    resumo["total_vendido"] = sum(
                        float(r[9] or 0) for r in vendas_evento
                    )
                    resumo["entregas_pendentes"] = sum(
                        1
                        for r in vendas_evento
                        if r[10] is None or not bool(r[10])
                    )

                    print(
                        f"[SGFE-API] vendas={len(vendas_evento)} "
                        f"evento={evento_id}",
                        flush=True
                    )
                    ...
                    try:
                        ...
                        pass
                    print(f"[SGFE-API] vendas: {exc}", flush=True)

                # -------------------------------------------------
                # 5. PAGAMENTOS
                # Busca os pagamentos existentes e cruza pelo IDVenda.
                # -------------------------------------------------
                pagamentos = {}
                try:
                    cur.execute("""
                    ...
                    for r in vendas_evento:
                        ...
                        status_pg = str(r[12] or "aberto").strip().lower()
                    ...
                    try:
                        ...
                        pass
                    print(f"[SGFE-API] pagamentos: {exc}", flush=True)
                ...
                try:
                    ...
                    """, [evento_id])
                    r_desp = cur.fetchone()
                    resumo["despesas"] = float(r_desp[0] or 0) if r_desp else 0.0
                except Exception as exc:
                    try:
                        conn.rollback()
                    except Exception:
                        pass
                    print(f"[SGFE-API] despesas: {exc}", flush=True)
        ...
        print(
            "[SGFE-API] EVENTO-GERAL V3 OK | "
            f"evento={evento_id} "
            f"agendamentos={resumo['agendamentos']} "
            f"vendas={resumo['vendas']} "
            f"entregas={resumo['entregas_pendentes']} "
            f"pend_pag={resumo['pagamentos_pendentes']} "
            f"vendido={resumo['total_vendido']} "
            f"recebido={resumo['total_recebido']} "
        ...
        print(f"[SGFE-API] EVENTO-GERAL V3 ERRO: {e}", flush=True)
