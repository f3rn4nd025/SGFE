...
def api_balizamento_marcar():
    ...
    try:
        ...
        # Primeiro verifica se JÁ EXISTE outro atleta marcado na mesma
        # PROVA + SÉRIE. Essa informação é usada somente para o alerta.
        # A marcação continua permitida normalmente.
        cur.execute("""
            SELECT
                M.IDMarcacao,
                M.IDCliente,
            ...
            WHERE M.IDEvento=?
              ...
              AND M.IDCliente<>?
            ORDER BY M.IDMarcacao ASC
        """, [id_evento, numero_prova, numero_serie, id_cliente])
        ...
        for r in cur.fetchall():
            outros_antes.append({
                ...
                "id_cliente": access_int(r[1]),
                "atleta": str(r[2] or ""),
                "raia": access_int(r[3]) if r[3] is not None else 0,
                "cor": str(r[4] or "amarelo"),
            })
        ...
        # Usa também a verificação feita ANTES da gravação para garantir
        # que o alerta apareça mesmo quando a marcação atual foi atualizada.
        # A cor final continua sendo calculada pela ordem das marcações.
        outros_alerta = outros if outros else outros_antes

        return jsonify({
            "ok": True,
            "marcacao": atual,
            "cor": atual["cor"] if atual else "amarelo",
            "conflito": bool(outros_alerta),
            "mesma_serie": bool(outros_alerta),
            "quantidade": len(marcados),
            "marcados": marcados,
            "outros_atletas": outros_alerta,
            "mensagem": (
                f"ATENÇÃO: esta série já possui "
                f"{len(outros_alerta)} atleta(s) marcado(s)."
                if outros_alerta
