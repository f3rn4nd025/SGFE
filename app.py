...
def web_vendas():
    ...
    try:
        ...
        # DIAGNOSTICO TEMPORARIO DA LISTA DE VENDAS
        # Mostra no log exatamente quais IDEvento existem em tblVendaPacotes.
        try:
            cur.execute("SELECT COUNT(*) FROM tblVendaPacotes")
            total_vendas = cur.fetchone()[0]
            cur.execute("SELECT IDVenda, IDEvento, IDAgendamento FROM tblVendaPacotes ORDER BY IDVenda DESC")
            diagnostico_vendas = cur.fetchall()
            print(f"[SGFE-VENDAS] evento_selecionado={evento_selecionado} total_vendas={total_vendas}")
            print(f"[SGFE-VENDAS] registros_IDEvento={diagnostico_vendas[:30]}")
        except Exception as diag_e:
            print(f"[SGFE-VENDAS] ERRO_DIAGNOSTICO={diag_e}")
            try:
                conn.rollback()
            except Exception:
                pass
